...
ENTITY_LIST_FILE_PATH = models.Variable.get('mdm_entity_list_path')
REGION =  models.Variable.get('gcp_mdm_region')
PROJECT_ID = models.Variable.get('gcp_mdm_project')
DATAPLEX_REGION= models.Variable.get('mdm_dplx_region')
LAKE_ID = models.Variable.get('mdm_dplx_lake_id')
RAW_ZONE_ID = models.Variable.get('mdm_dplx_raw_zone_id')
DATA_PRODUCT_ZONE_ID = models.Variable.get('mdm_dplx_data_product_zone_id')
TAG_TEMPLATE = models.Variable.get('tag_template_mdm_information') 
TAG_INPUT_FILE = models.Variable.get('tag_template_input_file')
TAG_INPUT_PATH = models.Variable.get('tag_template_input_path')
SUB_NETWORK=models.Variable.get('gcp_sub_net')
TAG_JAR=models.Variable.get('gdc_tag_jar')
DPLX_TASK_PREFIX=models.Variable.get('mdm_tag_prefix')
TAG_MAIN_CLASS=models.Variable.get('mdm_information_main_class')
...
logger = logging.getLogger('mdm_info_tag')
...
def get_clouddq_task_status(task_id):
    ...
    res = session.get(
        f"{DATAPLEX_ENDPOINT}/v1/projects/{PROJECT_ID}/locations/{DATAPLEX_REGION}/lakes/{LAKE_ID}/tasks/{task_id}/jobs", headers=headers)
...
def _get_dataplex_job_state(**kwargs):
    ...
    while (task_status != 'SUCCEEDED' and task_status != 'FAILED'):
        ...
        time.sleep(30)
        task_status = get_clouddq_task_status(kwargs['dplx_task_id'])
        print(f"Cloud Data Classification tag task status is {task_status}")
    return task_status+ "_{}".format(kwargs['entity_val'])


with models.DAG(
        'create_mdm_information_tag',
    ...
    #)
        ...
        create_dataplex_mdm_info_tag_task = DataplexCreateTaskOperator(
            ...
            body={
                ...
                "execution_spec": {
                    ...
                    "args": {"TASK_ARGS": f" --tag_template_id={TAG_TEMPLATE}, --project_id={PROJECT_ID}, --location={REGION}, --lake_id={LAKE_ID},--zone_id={DATA_PRODUCT_ZONE_ID}, --entity_id={entity},--input_file={TAG_INPUT_FILE}"
                             }
                },
                "spark": {
        ...
        start >> generate_uuid_dq_check >> create_dataplex_mdm_info_tag_task >> dataplex_task_state >> [dataplex_task_success,dataplex_task_failed] >> end
